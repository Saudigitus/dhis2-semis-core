...
try {
    ...
    // Detecta alterações no último commit
    ...
    SUBTREES.forEach(subtree => {
        ...
        try {
            // Windows safe: define variável para evitar loop
            execSync(
                `cmd /c "set SEMIS_SUBTREE_SYNC=1 && git subtree push --prefix=${subtree.folder} ${subtree.repo} ${currentBranch}"`,
                { stdio: 'inherit' }
            );

            console.log(`✅ ${subtree.folder} sincronizado`);
        } catch (err) {
            // Trata "no new revisions" como informação, não erro
            if (err.stdout?.toString().includes('no new revisions') || err.message.includes('no new revisions')) {
